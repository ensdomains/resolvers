...
contract CustodialPublicResolver is PublicResolver {
    ...
    function setMultihashOnBehalfOf(bytes32 node, bytes hash, uint256 nonce, address signer, bytes signature) public {

    }

    function setNameOnBehalfOf(bytes32 node, string name, uint256 nonce, address signer, bytes signature) public {

    }

    function setABIOnBehalfOf(
        bytes32 node,
        uint256 contentType,
        bytes data,
        uint256 nonce,
    ...
    function setPubkeyOnBehalfOf(bytes32 node, bytes32 x, bytes32 y, uint256 nonce, address signer, bytes signature)
        public
    {

    }

    function setTextOnBehalfOf(bytes32 node, string key, string value, uint256 nonce, address signer, bytes signature)
        public
    {

    }

    function validateSignature(bytes32 hash, address signer, bytes signature) private {
        require(!submitted[hash]);
        require(SignatureValidator.isValidSignature(hash, signer, signature));
        submitted[hash] = true;
    }
}
